...
test("管理后台具备持久化、鉴权和完整 CRUD 契约", async () => {
  ...
  assert.doesNotMatch(ecsAdminHtml, /name="accent"|data-accent-preview|>配色</);
  assert.match(ecsAdminHtml, /data-markdown-toolbar/);
  assert.match(ecsAdminHtml, /data-media-dialog/);
  assert.match(ecsAdminHtml, /data-paste-upload-panel/);
  assert.match(ecsAdminHtml, /data-cancel-paste-upload/);
  assert.match(ecsAdminHtml, /可直接粘贴截图或图片/);
  ...
  assert.doesNotMatch(ecsAdminScript, /updateAccentPreview|elements\.accent/);
  assert.match(ecsAdminScript, /\/api\/media\/policy/);
  assert.match(ecsAdminScript, /\/api\/media\/complete/);
  assert.match(ecsAdminScript, /uploadToOss/);
  assert.match(ecsAdminScript, /uploadFile/);
  assert.match(ecsAdminScript, /bodyInput\.addEventListener\("paste", handleBodyPaste\)/);
  assert.match(ecsAdminScript, /event\.clipboardData\?\.items/);
  assert.match(ecsAdminScript, /item\.kind === "file"/);
  assert.match(ecsAdminScript, /image\/jpeg/);
  assert.match(ecsAdminScript, /image\/png/);
  assert.match(ecsAdminScript, /image\/webp/);
  assert.match(ecsAdminScript, /createUploadPlaceholder/);
  assert.match(ecsAdminScript, /\\u2063/);
  assert.match(ecsAdminScript, /pasteQueue = pasteQueue\.then/);
  assert.match(ecsAdminScript, /cleanupPendingMedia/);
  assert.match(ecsAdminScript, /插入正文/);
  assert.doesNotMatch(ecsAdminScript, /navigator\.clipboard|readText\(|read\(\)/);
  assert.doesNotMatch(ecsAdminScript, /data:image\/|;base64/);
  ...
  assert.match(ecsServer, /const accent = category === "项目经验" \? "mint" : "sky"/);
  assert.doesNotMatch(ecsServer, /请选择有效的文章配色/);
  assert.match(adminClient, /accent: categoryAccent\[article\.category\]/);
