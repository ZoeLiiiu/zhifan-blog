...
export function parseArticleInput(
  ...
  if (value.body !== undefined && value.body.length > 100000) {
    return { error: "正文不能超过 100000 个字" };
  }
  if (value.body !== undefined && /data:[^,\s]+;base64,/i.test(value.body)) {
    return { error: "正文不能包含 Base64 文件，请使用媒体上传功能" };
  }
  ...
  if (value.contentFormat !== undefined && !isArticleContentFormat(value.contentFormat)) {
    return { error: "请选择有效的正文格式" };
  }
